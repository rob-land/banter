...
- **UI toolkit**: GTK4 + libadwaita (via PyGObject / GObject Introspection). Blueprint (`.blp`) under `data/ui/` for the structures that have been migrated; the rest is still constructed programmatically (most of `window.py`, every `widgets/*.py`, and most `dialogs/*.py`). Convert to `.blp` opportunistically — don't block on a flag-day rewrite.
